...
def build(args):
    ...
    if args.target and (
        args.target == "aarch64-linux-ohos" or args.target == "x86_64-linux-ohos"
    ):
        if IS_WINDOWS:
            if DEVECO_OH_NATIVE_HOME:
    ...
    cmake_command = (
        ["cmake", STDX_DIR, "-G", generator] + cmake_defs + generate_cmake_defs(args)
    )
...
def extract_libstdx(directory: str) -> bool:
