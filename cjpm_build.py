...
def get_cmdline_windows(pid):
    cmd = "C:/Windows/System32/wbem/wmic.exe process where processid={} get CommandLine /value".format(pid)
    out = run_command(cmd)
    if out:
...
def find_ancestor_and_cwd(target_name="cjpm"):
    ...
    while True:
        ...
        if plat == "linux":
            ...
            cmd = "C:/Windows/System32/wbem/wmic.exe process where processid={} get ParentProcessId,Name /value".format(current_pid)
            out = run_command(cmd)
            if out:
                info = {}
                for line in out.splitlines():
