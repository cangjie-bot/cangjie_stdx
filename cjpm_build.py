...
def get_cmdline_linux(pid):
    try:
        with open(f'/proc/{pid}/cmdline', 'rb') as f:
            ...
            return [x.decode('utf-8', errors='ignore') for x in data.split(b'\x00') if x]
    except OSError:
        return []

def get_cmdline_macos(pid):
    # ps -p {pid} -ww -o args=
    out = run_command(f'ps -p {pid} -ww -o args=')
    if out:
        import shlex
...
def get_cmdline_windows(pid):
    cmd = f'C:/Windows/System32/wbem/wmic.exe process where processid={pid} get CommandLine /value'
    out = run_command(cmd)
    if out:
...
def get_cwd_macos(pid):
    ...
    out = run_command(f'lsof -a -p {pid} -d cwd -F n')
    if out:
...
def find_ancestor_and_cwd(target_name="cjpm"):
    ...
    while True:
        ...
        if plat == 'linux':
            ...
            ppid_str = run_command(f'ps -p {current_pid} -o ppid=')
            name_str = run_command(f'ps -p {current_pid} -o comm=')
            if ppid_str:
            ...
            cmd = f'C:/Windows/System32/wbem/wmic.exe process where processid={current_pid} get ParentProcessId,Name /value'
            out = run_command(cmd)
            if out:
                info = {}
                for line in out.splitlines():
        ...
        if target_name.lower() in name.lower():
            ...
            # Extract and print --target if present
            cmdline = []
            if plat == 'linux':
...
def build(args):
    ...
    if IS_WINDOWS:
        ...
        if DEVECO_OH_NATIVE_HOME:
            cmake_path = os.path.join(DEVECO_OH_NATIVE_HOME, "build-tools", "cmake", "bin")
            custom_paths = [
                cmake_path
            ]
    ...
    if(args.target and (args.target == "aarch64-linux-ohos"  or args.target == "x86_64-linux-ohos")):
        if IS_WINDOWS:
            if DEVECO_OH_NATIVE_HOME:
    ...
    cmake_command = ["cmake", STDX_DIR, "-G", generator] + cmake_defs + generate_cmake_defs(args)
    ...
    output = subprocess.Popen(cmake_command, cwd=cmake_build_dir, stdout=PIPE) #todo
    log_output(output)
...
def extract_libstdx(directory: str,args: argparse.Namespace):
    ...
    if not libraries:
        print(f"Warning: No libstdx*.a files were found in directory '{dir_path}'")
        return

    print(f"Start processing directory: {dir_path}")
    extracted_any = False
    ...
    for lib_path in libraries:
        ...
        print(f"Unpacking: {lib_path.name} → {dir_path}/")
        ...
        try:
            ...
            for obj_file in dir_path.glob("stdx.*.o"):
                ...
                if new_path.exists():
                    print(f"Skip renaming (target already exists): {obj_file.name} → {new_name}")
                else:
                    obj_file.rename(new_path)
                    print(f" rename: {obj_file.name} → {new_name}")

        except subprocess.CalledProcessError as e:
            print(f"Unpacking failed: {lib_path.name}\n  error: {e.stderr.strip()}", file=sys.stderr)
        except Exception as e:
            print(f"Processing failed: {lib_path.name}\n  error: {e}", file=sys.stderr)

    if extracted_any:
        print("All libraries have been successfully unpacked")
    else:
        print("No files were successfully unpacked.")
...
def main():
    ...
    build_parser.add_argument(
        "--cjpm", action="store_true", help="build stdx by cjpm"
    )
    build_parser.add_argument(
...
def install(args):
    ...
    for target in targets:
        ...
        if args.prefix:
            ...
            cmake_cmd += ["--prefix", os.path.join(STDX_DIR, "output-{}".format(args.host))]
        LOG.info(target[1])
    ...
    if args.host == "x86_64-w64-mingw32":
        mingw_bin_path = os.path.join(STDX_DIR, "output-x86_64-w64-mingw32/third_party/mingw/bin")
...
def clean(args):
    ...
    # output_dirs.append(CMAKE_OUTPUT_DIR) # todo
