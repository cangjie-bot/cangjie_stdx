...
架构图：
![stdx架构图](figures/Stdx_Architecture_Diagram_zh.png)

- aspectCJ: `stdx.aspectCJ` 包提供了 `Cangjie` 中面向切面编程的相关注解。
- compress: 提供压缩解压功能。
- crypto: 密码学操作的工具库。
- encoding: 数据编码与解码的基础工具库。
- fuzz: `Fuzz` 技术是一种自动化软件测试方法。
- log: `log` 包提供了一个单一的日志API。
- logger: `logger` 包提供文本格式和 `JSON` 格式日志打印功能。
- net: 网络通信和安全传输。
- serialization: `serialization` 包提供了序列化和反序列化的能力。
- unittest: 在编写仓颉项目单元测试代码时提供输入序列化格式的测试数据的能力。
