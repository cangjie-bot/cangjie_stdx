...
static void* KeylessAcNewctx(void* provctx)
{
    ...
    void* ctx = DYN_OPENSSL_zalloc(sizeof(KeylessAsymcipherCtx), dynMsg);
    KeylessCheckDynMsg(dynMsg, "[keyless] asym newctx");
    return ctx;
}
...
static void KeylessAcFreectx(void* v)
{
    ...
    DynMsg* dynMsg = KeylessProviderGetThreadDynMsg();
    DYN_OPENSSL_secure_free(c, dynMsg);
    KeylessCheckDynMsg(dynMsg, "[keyless] asym freectx");
}
