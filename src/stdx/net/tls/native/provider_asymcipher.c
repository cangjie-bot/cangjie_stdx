 ...
 *
 * @note:
 *   - Designed for side-channel resistant code (no data-dependent branches).
 *   - The return value is a mask suitable for constant-time selection.
 *   - Relies on 32-bit unsigned int and two's complement arithmetic
 *     (unsigned right shifts are logical).
 */
static unsigned int CtIsZero8(unsigned int x)
{
 ...
 *    0x00u otherwise.
 */
static unsigned int CtEq8(unsigned int a, unsigned int b)
{
 ...
 *
 * @note:
 *   - The returned mask is unsigned int wide. If a size_t-wide mask is required,
 *     widen explicitly, e.g.: (size_t)0 - (size_t)(CtIsZeroSizeT(x) & 1u).
 *   - Valid for any size_t bit width.
 */
static unsigned int CtIsZeroSizeT(size_t x)
{
 ...
 */
static unsigned int CtEqSizeT(size_t a, size_t b)
{
 ...
 */
static unsigned int CtLtSizeT(size_t a, size_t b)
{
    size_t diff = a - b;
 ...
 */
static size_t CtSelectSizeT(unsigned int mask, size_t a, size_t b)
{
