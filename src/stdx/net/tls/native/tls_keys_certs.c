...
static EVP_PKEY* keyless_from_rsa_pubkey(EVP_PKEY* pub, const char* key_id, DynMsg* dynMsg)
{
    ...
    OSSL_PARAM params[] = {DYN_OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_RSA_N, n_buf, nLen, dynMsg),
                           ...
                           DYN_OSSL_PARAM_construct_utf8_string("KEYLESS_ID", (char*)key_id, 0, dynMsg),
...
static EVP_PKEY* keyless_from_ec_pubkey(EVP_PKEY* pub, const char* key_id, DynMsg* dynMsg)
{
...
static EVP_PKEY* MakeKeylessFromPubkey(EVP_PKEY* pub, const char* key_id, DynMsg* dynMsg)
{
    int type = DYN_EVP_PKEY_get_id(pub, dynMsg);
    if (type == EVP_PKEY_RSA) {
        return keyless_from_rsa_pubkey(pub, key_id, dynMsg);
    }
    if (type == EVP_PKEY_EC) {
        return keyless_from_ec_pubkey(pub, key_id, dynMsg);
    }
    KEYLESS_PROVIDER_LOG("[keyless] invalid type: %d\n", type);
    return NULL;
}

EVP_PKEY* CreateKeylessKeyFromCtx(SSL_CTX* ctx, const char* key_id, DynMsg* dynMsg)
{
    ...
    if (!ctx || !key_id) {
        KEYLESS_PROVIDER_LOG("[keyless] invalid args: ctx/key_id\n");
        goto out;
    }
    ...
    keyless = MakeKeylessFromPubkey(pub, key_id, dynMsg);
    if (!keyless) {
...
extern void CJ_TLS_FreeKeylessId(char* key_id)
{
    free(key_id);
}
