...
static _Atomic int g_lib_num = 0;

int keyless_err_lib_init(void)
{
    int v = atomic_load(&g_lib_num);
    if (v != 0) {
        return v;
    }
    ...
    int expected = 0;
    if (atomic_compare_exchange_strong(&g_lib_num, &expected, assigned)) {
        return assigned;
    }
    /* Lost race, return existing */
    return atomic_load(&g_lib_num);
}
 ...
 *  - The leftmost (8*emLen - em_bits) bits of maskedDB are cleared.
 */
static int EncodePss(unsigned char* em, size_t emLen, size_t em_bits, const unsigned char* mhash, size_t hashLen, const EVP_MD* hashMd, const EVP_MD* mgf1Md, int saltLen, DynMsg* dynMsg)
{
    ...
    size_t salt_sz = (size_t)saltLen;
    if (emLen < hashLen + salt_sz + 2) {
        return 0;
    }
    ...
    if (!KeylessPssPrepareBuffers(dbLen, salt_sz, &salt, &db, &dbmask, dynMsg)) {
        goto done;
    }
    if (!KeylessPssFillSalt(salt, salt_sz, dynMsg)) {
        goto done;
    }
    if (!KeylessPssComputeDigest(hashMd, mhash, hashLen, salt, salt_sz, digest, dynMsg)) {
        goto done;
    }
    if (!KeylessPssPopulateDb(db, dbLen, salt, salt_sz)) {
        goto done;
    }
    ...
    KeylessPssCleanupBuffers(salt, salt_sz, db, dbmask, dbLen, dynMsg);
    DYN_OPENSSL_cleanse(digest, sizeof(digest), dynMsg);
    return ok;
}
...
static const Pkcs1DigestDescriptor pkcs1_digest_table[] = {{"sha1", PKCS1_SHA1_PREFIX, sizeof(PKCS1_SHA1_PREFIX), 20},
...
static const Pkcs1DigestDescriptor* LookupDigestDescriptor(const char* name)
{
    ...
    for (const Pkcs1DigestDescriptor* d = pkcs1_digest_table; d->name != NULL; ++d) {
...
static int BuildRemoteAlg(KeylessSignCtx* c)
{
    ...
    if (c->type == KEYLESS_KEY_TYPE_ECDSA) {
...
static unsigned bits_from_uncompressed_point(const unsigned char* pt, size_t len)
{
    if (!pt || len < 1) {
        return 0;
    }
...
static unsigned keylesskey_curve_bits(const KeylessKey* k)
{
    ...
    bits = bits_from_uncompressed_point(k->ecPoint, k->ecPointLen);
    return bits;
}

/* Return a safe upper bound for DER-encoded ECDSA signature length
 * for common prime curves based on curve bits. */
static size_t ecdsa_der_sig_maxLen_from_bits(unsigned bits)
{
...
static int KeylessEcdsaSign(KeylessSignCtx* c, KeylessRemoteSignCb cb, unsigned char* sig, size_t* siglen, size_t sigsize, const unsigned char* tbs, size_t tbslen)
{
    ...
    unsigned bits = keylesskey_curve_bits((KeylessKey*)(c->keyData));
    size_t max_need = ecdsa_der_sig_maxLen_from_bits(bits);
    ...
    char* key_id = ((KeylessKey*)(c->keyData))->keyId;
    KEYLESS_PROVIDER_LOG("[keyless] invoking remote ECDSA alg=%s dlen=%zu cap=%zu\n", c->algName, tbslen, sigsize);

    int64_t got = 0;
    unsigned char* out = (unsigned char*)cb(key_id, c->algName, tbs, (int64_t)tbslen, &got);
    if (!out || got <= 0) {
