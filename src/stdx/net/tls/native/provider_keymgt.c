...
static int KeylessHas(const void* keyData, int selection)
{
    ...
    int ok = 1;
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) {
        ok &= ((k->type == 1 && k->n && k->e) || (k->type == 2 && k->ecPoint));
    }
    if (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) {
        /* We claim private key capability (remotely) so signature operations proceed */
        ok &= 1;
    }
    return ok;
}
 ...
 *
 * Parameters:
 *  - keyData1: Pointer to the first provider-specific key data instance.
 *  - keyData2: Pointer to the second provider-specific key data instance to compare against.
 *  - selection: Bitmask indicating which properties must match (e.g., algorithm,
 *               parameters, key size, public key). When used with OpenSSL-style
 *               providers, this typically corresponds to OSSL_KEYMGMT_SELECT_* flags.
 *
 * Returns:
 ...
 *
 * Notes:
 *  - The function does not take ownership of the provided pointers.
 *  - Intended to operate without private key material; only public attributes and metadata relevant to 'selection' are considered.
 */
    ...
    if (k->type == KEYLESS_KEY_TYPE_RSA) {
        ...
        if (orderBits) {
            if ((p = DYN_OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_MAX_SIZE, NULL))) {
                ...
                size_t der_sig_max = 2 * nbytes + 10;
                DYN_OSSL_PARAM_set_size_t(p, der_sig_max, NULL);
            }
        } else {
            if ((p = DYN_OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_MAX_SIZE, NULL))) {
                DYN_OSSL_PARAM_set_size_t(p, 160, NULL); /* > P-521 ECDSA DER max (~142B) */
            }
        }
    }
