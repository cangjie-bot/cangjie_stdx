...
main() {
    ...
    // 打印结果 
    println("使用pretty配置的JSON输出: ${String.fromUtf8(buffer.bytes())}")
}
```
...
main() {
    ...
    // 打印结果 
    println("修改DateTime格式的JSON输出: ${String.fromUtf8(buffer.bytes())}")
}
```
...
main() {
    ...
    // 打印结果 
    println("启用htmlSafe的JSON输出: ${String.fromUtf8(buffer.bytes())}")
}
```
...
main() {
    ...
    let buffer = ByteBuffer()
    let writer = JsonWriter(buffer)
    var config = WriteConfig.pretty // 紧凑格式无换行行为，缩进/填充不生效
    ...
    // 打印结果 
    println("使用indent格式的JSON输出: ${String.fromUtf8(buffer.bytes())}")
}
```
...
main() {
    ...
    // 打印结果 
    println("使用newline格式的JSON输出: ${String.fromUtf8(buffer.bytes())}")
}
```
