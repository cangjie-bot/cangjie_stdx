...
public class Diagnostic {
    public prop diagInfo: DiagnosticInfo
}
```
...
功能：获取当前代码诊断信息的具体诊断信息，包括 `Warning` 类型等。
...
```cangjie
public prop diags: Array<Diagnostic>
```
...
类型：[SyntaxTreeNode](#class-syntaxtreenode) 的子类型 `T`。
...
```cangjie
sealed abstract class SyntaxTreeNode <: ToString {
...
- [CodePositionRange](syntax_package_structs.md#struct-codepositionrange) - 返回所有 `catch` 关键字的位置。
