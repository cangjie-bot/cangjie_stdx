...
main() {
    ...
    let signature = signatureOpt.getOrThrow()

    // 核心演示：获得证书签名的二进制
    let signatureValue = signature.signatureValue
    println("Signature DerBlob size: ${signatureValue.size}")
}
```
...
main() {
    ...
    // 获取签名的哈希值
    let hashCode = signature.hashCode()
    println("Signature HashCode: ${hashCode}")
}
```
...
main() {
    ...
    let sig1 = sig1Opt.getOrThrow()
    let sig2 = sig2Opt.getOrThrow()

    // 比较两个签名是否不相等
    let isNotEqual = sig1 != sig2
...
main() {
    ...
    let signature1 = signatureOpt.getOrThrow()
    // 重新获取同一个证书的签名
    for (cert in certs) {
        if (cert.issuer.commonName == "ISRG Root X1") {
            let x509Certificate: X509Certificate = cert
    ...
    let signature2 = signatureOpt.getOrThrow()

    // 比较两个签名是否相等
    let isEqual = signature1 == signature2
...
main() {
    ...
    // 验证证书
    let isValid = cert.verify(verifyOpt)

    println("证书是否有效: ${isValid}")
    ...
    verifyOpt2.dnsName = ""
    // 验证证书
    let isValid2 = cert.verify(verifyOpt2)

    println("证书是否有效: ${isValid2}")
}
```
...
main() {
    ...
    // 输出证书请求的实体可辨识名称
    println("Subject: ${certRequestInfo.subject.getOrThrow()}")
}
```
