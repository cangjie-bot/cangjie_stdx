...
main() {
    ...
    // 核心演示：将证书编码为DER格式
    let derBlob = cert.encodeToDer()

    println("DER格式证书数据长度: ${derBlob.size}")
}
```
...
main() {
    ...
    println("PEM格式证书编码后字符串长度: ${pemEntry.encode().size}")
}
```
...
main() {
    ...
    // 核心演示：获取证书哈希值
    let hashCode = cert.hashCode()

    println("证书哈希值: ${hashCode}")
}
```
...
main() {
    ...
    // 核心演示：获取证书字符串表示
    let certStr = cert.toString()

    println("证书字符串表示: ${certStr}")
}
```
...
main() {
    ...
    // 获取到两个不同的证书对象
    let cert1 = certOpt1.getOrThrow()
    let cert2 = certOpt2.getOrThrow()

    // 核心演示：比较两个证书是否不相等
    let isNotEqual = cert1 != cert2

    println("两个不同证书是否不相等: ${isNotEqual}")
    
    // 比较同一个证书是否不相等
    let isNotEqualSelf = cert1 != cert1
...
main() {
    ...
    // 核心演示：比较两个不同证书是否相等
    let isEqual = cert1 == cert2

    println("两个不同证书是否相等: ${isEqual}")
    
    // 比较同一个证书是否相等
    let isEqualSelf = cert1 == cert1
