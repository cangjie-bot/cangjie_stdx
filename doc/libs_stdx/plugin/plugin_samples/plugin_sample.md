...
open class CheckDeriveUsagePlugin {
...
@astPlugin[AfterSema]
open class CheckConstVarNamePlugin {
    public prop name: String {
        get() {
            "checkConstVarName"
        }
            ...
            for (decl in file.topLevelDecls) {
                if (let v: VarDecl <- decl && let Const <- v.kind) {
                    ...
                    if (v.name.toUpper() != v.name) {
                        addDiagnostic(DiagnosticInfo.Error("const_var_not_uppercase", "const var name is not all uppercase: ${v.name}, should be ${v.name.toUpper()}"), v.nodePos.filePath, v.nodePos.beginLine, v.nodePos.beginColumn, v.nodePos.endLine, v.nodePos.endColumn)
                        return false
                    }
                }
            }
