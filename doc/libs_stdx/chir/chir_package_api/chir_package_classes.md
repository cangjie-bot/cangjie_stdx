...
功能：表示类型系统中的布尔类型。这是一个单例类型，表示内置的布尔类型。

父类型：

- [BuiltinType](#class-builtintype)
- Equatable\<BoolType> - 提供相等性比较功能
...
功能：内置类型的抽象基类。该类表示语言中内置的类型，如原始数值类型、布尔类型、单元类型等。

父类型：

- [Type](#class-type)
- Equatable\<BuiltinType> - 提供相等性比较功能
...
功能：检查该内置类型是否与另一个内置类型相等。

参数：

- other: BuiltinType - 要比较的内置类型。
...
main() {
    let intType1 = IntType.getI32()
    let intType2 = IntType.getI32()
    let floatType = FloatType.getF32()
...
- [BuiltinType](#class-builtintype)
- Equatable\<CPointerType> - 提供相等性比较功能
...
- [BuiltinType](#class-builtintype)
- Equatable\<CStringType> - 提供相等性比较功能
...
main() {
    let float16Type = FloatType.getF16()
...
main() {
    let float32Type = FloatType.getF32()
...
main() {
    let float64Type = FloatType.getF64()
...
main() {
    let float32Type1 = FloatType.getF32()
    let float32Type2 = FloatType.getF32()
    let float64Type = FloatType.getF64()
...
main() {
    let intNativeType = IntType.getINative()
...
main() {
    let uintNativeType = IntType.getUNative()
...
main() {
    let int32Type1 = IntType.getI32()
    let int32Type2 = IntType.getI32()
    let int64Type = IntType.getI64()
...
- [BuiltinType](#class-builtintype)
- Equatable\<NothingType> - 提供相等性比较功能
...
- [BuiltinType](#class-builtintype)
- Equatable\<NumericType> - 提供相等性比较功能
...
- Bool - 如果类型是内置类型则返回 true，否则返回 false。
...
main() {
    let intType = IntType.getI32()
    println("Int32 是内置类型: ${intType.isBuiltinType()}")
}
```
...
main() {
    let type1 = IntType.getI32()
    let type2 = IntType.getI32()
    let type3 = IntType.getI64()
