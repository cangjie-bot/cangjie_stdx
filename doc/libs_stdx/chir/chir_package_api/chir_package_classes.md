...
```cangjie
public class BoolType <: ExtensibleType & Equatable<BoolType>
```
...
- [ExtensibleType](#class-extensibletype)
- Equatable\<BoolType>
...
```cangjie
public class CPointerType <: ExtensibleType & Equatable<CPointerType>
```
...
```cangjie
public class CStringType <: ExtensibleType & Equatable<CStringType>
```
...
```cangjie
sealed abstract class ExtensibleType <: Type & Equatable<ExtensibleType>
```

功能：可扩展类型的抽象基类。该类表示可以扩展的类型，包括原始类型、数值类型、布尔类型、单元类型以及复合类型（如类、枚举、结构体）等。

父类型：

- [Type](#class-type)
- Equatable\<ExtensibleType>

### operator func==(ExtensibleType)

```cangjie
public operator func==(other: ExtensibleType): Bool
```

功能：检查该可扩展类型是否与另一个可扩展类型相等。

参数：

- other: ExtensibleType - 要比较的可扩展类型。
...
```cangjie
public class NothingType <: ExtensibleType & Equatable<NothingType>
```
...
```cangjie
sealed abstract class NumericType <: ExtensibleType & Equatable<NumericType>
```
...
```cangjie
public class RuneType <: ExtensibleType & Equatable<RuneType>
```
...
### func isExtensibleType()

```cangjie
public func isExtensibleType(): Bool
```

功能：检查该类型是否为可扩展类型。

可扩展类型包括原始类型（Int、Float、Bool、Rune、Unit、Nothing）、C 类型（CPointer、CString）以及复合类型（Class、Enum、Struct）。

返回值：

- Bool - 如果类型是可扩展类型则返回 true，否则返回 false。
...
main() {
    let intType = IntType.getInt32()
    println("Int32 是可扩展类型: ${intType.isExtensibleType()}")
}
```
...
```cangjie
public class UnitType <: ExtensibleType & Equatable<UnitType>
```
