...
Functionality: This class provides deserialization capability from JSON data streams to Cangjie objects.
...
Functionality: Retrieves the type of the next [JsonToken](encoding_json_stream_package_enums.md#enum-jsontoken) from the input stream, without guaranteeing the correctness of its format.
...
- Option\<[JsonToken](encoding_json_stream_package_enums.md#enum-jsontoken)> - The type of the next [JsonToken](encoding_json_stream_package_enums.md#enum-jsontoken), or None if the end of the input stream is reached.

Exceptions:

- IllegalStateException - Thrown if the next character in the input stream is not one of: (n, t, f, ", 0-9, -, {, }, [, ]).
...
>
> - When the next [JsonToken](encoding_json_stream_package_enums.md#enum-jsontoken) is one of [JsonInt](../../json/json_package_api/encoding_json_package_classes.md#class-jsonint), [JsonFloat](../../json/json_package_api/encoding_json_package_classes.md#class-jsonfloat), [JsonBool](../../json/json_package_api/encoding_json_package_classes.md#class-jsonbool), or [JsonNull](../../json/json_package_api/encoding_json_package_classes.md#class-jsonnull), the raw string of the next `value` field will be read and returned.
>
...
[JsonWriter](encoding_json_stream_package_classes.md#class-jsonwriter) should be used in conjunction with the interface [JsonSerializable](encoding_json_stream_package_interfaces.md#interface-jsonserializable). [JsonWriter](encoding_json_stream_package_classes.md#class-jsonwriter) can use writeValue to write types implementing [JsonSerializable](encoding_json_stream_package_interfaces.md#interface-jsonserializable) to the Stream.

> **Note:**
>
> [JsonWriter](encoding_json_stream_package_classes.md#class-jsonwriter) uses caching to reduce IO operations when writing to the Stream. Before finishing with [JsonWriter](encoding_json_stream_package_classes.md#class-jsonwriter), the flush function must be called to ensure all cached data is written to the Stream.
...
Functionality: Constructor, creates an instance that writes data to the specified output stream.
...
Functionality: Ends serialization of the current JSON array.
...
Functionality: Ends serialization of the current JSON object.

Exceptions:

- IllegalStateException - Thrown when the current writer state does not expect to end a JSON object.
...
Functionality: Writes cached data to the output stream and calls the flush method of the output stream.
...
> **Note:**
>
> This function does not escape the value or add double quotes to the input. If the caller can ensure the input value conforms to the data conversion standard [ECMA-404 The JSON Data Interchange Standard](https://www.ecma-international.org/publications-and-standards/standards/ecma-404/), it is recommended to use this function.

Return value:

- [JsonWriter](encoding_json_stream_package_classes.md#class-jsonwriter) - Returns the current [JsonWriter](encoding_json_stream_package_classes.md#class-jsonwriter) reference for method chaining.

Exceptions:

- IllegalStateException - Thrown when the current writer state does not expect to write a value.
...
- IllegalStateException - Thrown when the current writer state does not expect to write a JSON array.
...
- IllegalStateException - Thrown when the current writer's state is not suitable for writing a JSON object.
...
- IllegalStateException - Thrown when the current [JsonWriter](encoding_json_stream_package_classes.md#class-jsonwriter) state is not suitable for writing the string specified by parameter `name`.
...
Function: Writes a JSON value `null` to the stream.
...
- IllegalStateException - Thrown when the current writer's state is not suitable for writing a value.
...
The json.stream package has extended [JsonSerializable](encoding_json_stream_package_interfaces.md#interface-jsonserializable) implementations for basic types (Int64, UInt64, Float64, Bool, String) and Collection types (Array, ArrayList, HashMap).
