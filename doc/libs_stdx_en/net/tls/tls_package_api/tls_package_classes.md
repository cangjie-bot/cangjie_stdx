...
> **Note:**
>
> Currently only supports three cipher suites: ECDHE-RSA-AES256-GCM-SHA384, TLS_AES_256_GCM_SHA384, TLS_CHACHA20_POLY1305_SHA256. Availability of other cipher suites is not guaranteed.
...
Function: Callback function during handshake process, providing TLS initial key data for debugging and decrypting records.
...
- [TlsException](../common/tls_common_package_api/tls_common_package_exceptions.md#class-tlsexception) - Thrown when the set server certificate is not of [X509Certificate](../../../crypto/x509/x509_package_api/x509_package_classes.md#class-x509certificate) type; thrown when the server certificate and corresponding private key file are set to None.
...
Function: Sets or gets the authentication mode required by the server from the client. Default value is [TlsClientIdentificationMode](tls_package_enums.md#enum-tlsclientidentificationmode).Disable, meaning the client is not required to authenticate the server certificate, nor to send its own certificate.
...
Function: Specifies the server's DH key parameters. Default is `None`, using OpenSSL auto-generated parameter values by default.
...
Function: Specifies the server's security level. Default value is 2, with optional parameter values in the range [0,5]. For parameter meanings, refer to [openssl-SSL_CTX_set_security_level](https://www.openssl.org/docs/man1.1.1/man3/SSL_CTX_set_security_level.html) documentation.
...
Function: Application Layer Protocol Negotiation protocols. If the client attempts to negotiate these protocols, the server will select the intersecting protocol names. If the client does not attempt protocol negotiation, this configuration will be ignored.
...
- IllegalArgumentException - Thrown when setting cipher suites through a `Map` parameter, if a cipher suite string for a TLS version contains null character `\0`.
