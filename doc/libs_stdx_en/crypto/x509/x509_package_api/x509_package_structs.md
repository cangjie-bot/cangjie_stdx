...
- Int64 - The result obtained after hashing the [DerBlob](x509_package_structs.md#struct-derblob) object.
...
- Bool - Returns true if objects are different; otherwise, returns false.
...
- Bool - Returns true if objects are identical; otherwise, returns false.
...
Functionality: Digital certificate extension fields typically contain extended key usage descriptions. Currently supported usages include: ServerAuth, ClientAuth, EmailProtection, CodeSigning, OCSPSigning, and TimeStamping.
...
Functionality: Constructs extended key usage with specified purposes. Note that a single key can have multiple usages.
...
Functionality: Digital certificate extension fields typically contain descriptions of public key usages. Currently supported usages include: DigitalSignature, NonRepudiation, KeyEncipherment, DataEncipherment, KeyAgreement, CertSign, CRLSign, EncipherOnly, and DecipherOnly.
...
Functionality: Indicates the private key can be used to sign CRLs, while the public key can verify CRL signatures.
...
Functionality: Indicates the private key is used for certificate signing, while the public key verifies certificate signatures. Specifically for CA certificates.
...
Functionality: Indicates the public key in the certificate is only used for decryption during key agreement. Meaningful only when used with KeyAgreement.
...
Functionality: Indicates the private key can be used for various digital signature operations (except certificate signing, CRL signing, and non-repudiation services), while the public key verifies these signatures.
...
Functionality: Indicates the public key in the certificate is only used for encryption during key agreement. Meaningful only when used with KeyAgreement.
...
Function: Indicates the key is used to encrypt other keys for transmission.
...
Function: Indicates the private key can be used for signing in non-repudiation services, while the public key is used to verify signatures.
...
Function: Constructs an extended key usage with specified purposes. Note that a single key can have multiple purposes.

Parameters:

- keys: UInt16 - The key usage. It is recommended to use the key usage variables provided in this structure, combined via bitwise OR operations.
...
Function: The [Pem](x509_package_structs.md#struct-pem) struct is a sequence of entries and can contain multiple [PemEntry](x509_package_structs.md#struct-pementry) items.
...
Function: Returns a PEM-formatted string. Line endings are generated according to the current operating system.
...
Function: Determines if the PEM text decoded into a sequence of entries is empty.

Return Value:

- Bool - Returns true if the decoded PEM text sequence is empty; otherwise, returns false.
...
Function: Generates an iterator for the PEM text decoded into a sequence of entries.

Return Value:

- Iterator\<[PemEntry](x509_package_structs.md#struct-pementry)> - The iterator for the PEM text decoded into a sequence of entries.
...
The last line is a UTF-8 encoded string composed of "-----END", a label, and "-----", as described in [RFC 1421](https://www.rfc-editor.org/rfc/rfc1421.html).
In older versions of the PEM encoding standard, entry headers were included between the first line and the body.

To support different user scenarios, we provide the [PemEntry](x509_package_structs.md#struct-pementry) and [Pem](x509_package_structs.md#struct-pem) types. [PemEntry](x509_package_structs.md#struct-pementry) is used to store a single PEM basic structure.
...
Function: Indicates the entry type is a certificate.
...
Function: Indicates the entry type is a certificate signing request.
...
Function: Indicates the entry type is DH key parameters.
...
Function: Indicates the entry type is elliptic curve parameters.
...
Function: Indicates the entry type is an elliptic curve private key.
...
Function: Indicates the entry type is a PKCS #8 standard encrypted private key.
...
Function: Indicates the entry type is a PKCS #8 standard unencrypted private key.
...
Function: Indicates the entry type is a public key.
...
Function: Indicates the entry type is an RSA private key.
...
Function: Indicates the entry type is an SM2 private key.
...
Function: Indicates the entry type is a certificate revocation list.
...
Function: Binary content of the [PemEntry](x509_package_structs.md#struct-pementry) instance.
...
Function: Headers of the [PemEntry](x509_package_structs.md#struct-pementry) instance.
...
Function: Label of the [PemEntry](x509_package_structs.md#struct-pementry) instance.
...
Function: Constructs a [PemEntry](x509_package_structs.md#struct-pementry) object.
...
Function: Finds corresponding header content by header name.
...
- Iterator\<String> - An iterator for the content corresponding to the header name.
...
Function: Returns the label and binary content length of the PEM object.
...
Function: Generates a random serial number of specified length.
...
- [X509Exception](./x509_package_exceptions.md#class-x509exception) - Throws an exception when length equals 0 or exceeds 20.
...
Function: Returns the hash value of the certificate serial number.
...
Function: Generates a hexadecimal string representation of the certificate serial number.

Return value:

- String - The certificate serial number string.
...
- Bool - Returns true if the serial numbers are identical; otherwise, returns false.
...
Function: The signature of a digital certificate, used to verify identity authenticity.
...
Function: Returns the binary data of the certificate signature.
...
Function: Returns the hash value of the certificate signature.
...
- Bool - Returns true if the certificate signatures are different; otherwise, returns false.
...
- Bool - Returns true if the certificate signatures are identical; otherwise, returns false.
...
Function: Provides configuration options for the `x509` certificate verification function [verify](./x509_package_classes.md#func-verifyverifyoption).
...
Function: Domain name for verification, empty by default. Domain name verification is only performed when this field is set.
...
Function: Intermediate certificate chain, empty by default.
...
Function: Root certificate chain, defaults to the system root certificate chain.
...
Function: Verification timestamp, defaults to the current time.

Type: DateTime```cangjie
public var time: DateTime = DateTime.now()
```
...
- serialNumber!: ?[SerialNumber](x509_package_structs.md#struct-serialnumber) - The serial number of the digital certificate. Default value is None. When using the default value, the default serial number length is 128 bits.
- notBefore!: ?DateTime - The start time of the digital certificate's validity period. Default value is None. When using the default value, the default time is the creation time of [X509CertificateInfo](x509_package_structs.md#struct-x509certificateinfo).
- notAfter!: ?DateTime - The expiration time of the digital certificate's validity period. Default value is None. When using the default value, the default time is 1 year after notBefore.
- subject!: ?[X509Name](x509_package_classes.md#class-x509name) - The subject information of the digital certificate. Default value is None.
- dnsNames!: Array\<String> - A list of domain names. Users must ensure the validity of input domain names. Default value is an empty string array.
- emailAddresses!: Array\<String> - A list of email addresses. Users must ensure the validity of input emails. Default value is an empty string array.
- IPAddresses!: Array\<[IP](x509_package_type.md#type-ip)> - A list of [IP](x509_package_type.md#type-ip) addresses. Default value is an empty [IP](x509_package_type.md#type-ip) array.
- keyUsage!: ?[KeyUsage](x509_package_structs.md#struct-keyusage) - Key usage. Default value is None.
- extKeyUsage!: ?[ExtKeyUsage](x509_package_structs.md#struct-extkeyusage) - Extended key usage. Default value is None.

Exceptions:

- [X509Exception](./x509_package_exceptions.md#class-x509exception) - Throws an exception if the input [IP](x509_package_type.md#type-ip) address list contains invalid [IP](x509_package_type.md#type-ip) addresses.
...
Function: The [X509CertificateRequestInfo](x509_package_structs.md#struct-x509certificaterequestinfo) structure contains certificate request information, including the subject distinguished name, domain names, email addresses, and [IP](x509_package_type.md#type-ip) addresses.
